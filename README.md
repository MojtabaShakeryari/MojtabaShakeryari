# Hi, I'm Mojtaba Shakeryari 👋

## Environmental Scientist | Remote Sensing • GIS • Earth Observation • Machine Learning

![Python](https://img.shields.io/badge/Python-3.11-blue)
![Google Earth Engine](https://img.shields.io/badge/Google%20Earth%20Engine-Remote%20Sensing-green)
![Remote Sensing](https://img.shields.io/badge/Remote%20Sensing-Satellite%20Data-orange)
![GIS](https://img.shields.io/badge/GIS-Spatial%20Analysis-yellow)
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-Random%20Forest-red)
![Jupyter](https://img.shields.io/badge/Jupyter-Notebook-F37626)

---

## Academic Background

**M.Sc. in Environmental Science**  
**University of Tehran, Iran**

GPA: **4.0/4.0**

My research focuses on the application of satellite remote sensing, GIS, climate data analysis, and machine learning techniques for environmental monitoring, ecosystem assessment, and climate change studies.

---
## About Me

I am an Environmental Scientist with research interests in satellite remote sensing, GIS, Earth observation, climate change, and machine learning applications for environmental monitoring. My work focuses on integrating multi-source geospatial datasets with data-driven approaches to investigate wetlands, urban environments, and climate-related environmental processes.

I am currently expanding my expertise in Python and Google Earth Engine to develop reproducible workflows for large-scale environmental analysis and decision support.


# Research Interests

- Remote Sensing and Earth Observation
- Geographic Information Systems (GIS)
- Wetland Monitoring and Ecosystem Assessment
- Climate Change Impacts
- Drought and Water Resources Analysis
- Environmental Data Science
- Machine Learning Applications in Environmental Modelling

---

# Master Thesis

## Assessment of Long-term Changes in Hamoun International Wetland and Their Relationship with Dust Storm Occurrence Using Satellite Remote Sensing Data

**University of Tehran**

Research methods:

- Landsat satellite data analysis (1977–2014)
- Wetland change detection
- Dust storm occurrence assessment
- Machine learning classification:
  - Support Vector Machine (SVM)
  - Artificial Neural Network (MLP)
  - Fuzzy ARTMAP
- NDVI analysis
- Mann–Kendall trend analysis
- Sen’s slope estimator

---

# Featured Projects 🌍

# Lake Urmia Environmental Monitoring (2016–2025)

An integrated satellite remote sensing and machine learning framework for assessing Lake Urmia water dynamics and climate drivers.

This project investigates long-term changes in lake surface water extent and evaluates relationships between water dynamics and climatic variables using Earth Observation data and environmental modelling approaches.

## Objectives

- Monitoring Lake Urmia surface water changes (2016–2025)
- Assessing climate drivers influencing lake dynamics
- Integrating satellite and climate datasets
- Developing predictive machine learning models

## Data Sources

🛰️ Sentinel-2 MSI satellite imagery  
🌎 Google Earth Engine workflow  
🌧️ CHIRPS precipitation dataset  
🌡️ ERA5-Land temperature data  
💧 MOD16 evapotranspiration dataset  
📉 SPEI drought indicators  

## Methods

- MNDWI and NDVI water extraction
- Satellite image preprocessing
- Time-series analysis
- Climate-water relationship assessment
- Random Forest modelling
- Regression analysis
- Future forecasting

## Main Outputs

- Lake surface water change analysis
- Climate variability assessment
- Machine learning model evaluation
- Environmental prediction framework
- Publication-style scientific figures

Repository:

➡️➡️ [Lake Urmia Environmental Monitoring](https://github.com/MojtabaShakeryari/Lake-Urmia-Climate-Water-Analysis)

---

# Publications & Research Papers

## 1. Wetland Remote Sensing and Drought Monitoring (2021)

**Shakeryari, M., et al. (2021).**

Monitoring of wetland changes affected by drought using four Landsat satellite data and Fuzzy ARTMAP classification method (case study Hamoun wetland, Iran).

**Arabian Journal of Geosciences**

DOI:
https://doi.org/10.1007/s12517-020-06320-8


---

## 2. Urban Climate and Air Pollution Modelling (2023)

**Shakeryari, M., et al. (2023).**

Detecting local climate zone change and its effects on PM10 distribution using fuzzy machine learning in Tehran, Iran.

**Urban Climate**

DOI:
https://doi.org/10.1016/j.uclim.2023.101506


---

## 3. GIS-Based Urban Resilience Assessment (2024)

**Hosseini, J., Shakeryari, M., et al. (2024).**

Comparison of the Analytic Network Process and the Best–Worst Method in Ranking Urban Resilience and Regeneration Prioritization by Applying Geographic Information Systems.

**Land, 13(7), 1008**

DOI:
https://doi.org/10.3390/land13071008


---

# Book Translation 📚

**Shakeryari, M. (Translator). (2017)**

## Economic Valuation of Wetlands

Topics:

- Wetland ecosystem services
- Environmental economics
- Conservation strategies
- Environmental management

---

# Technical Skills

## Remote Sensing & GIS

- Google Earth Engine (GEE)
- Sentinel-2
- Landsat
- MODIS
- ArcGIS
- ENVI
- TerrSet / IDRISI


## Programming & Data Science

- Python
- Jupyter Notebook
- Pandas
- NumPy
- Rasterio
- Matplotlib
- Scikit-learn


## Environmental Modelling

- Time-series analysis
- Climate data analysis
- Drought indices (SPEI)
- Spatial modelling
- Machine learning prediction
- Environmental monitoring


---

# Current Research Focus 🔬

- Satellite-based wetland monitoring
- Climate change impacts on aquatic ecosystems
- Earth Observation applications
- Remote sensing time-series analysis
- Machine learning for environmental prediction
- GIS-based environmental modelling


---


## Contact

- **GitHub:** https://github.com/MojtabaShakeryari
- **ORCID:** https://orcid.org/0009-0001-2131-3570
- **Google Scholar:** https://scholar.google.com/citations?user=tn-LPnEAAAAJ&hl=en
- **LinkedIn:** https://www.linkedin.com/in/mojtaba-shakeryari-92561733b/
